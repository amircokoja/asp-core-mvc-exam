...
function DodajAjaxEvente() {
    ...
    $(".ocjenaPromjena").on('input propertychange', function (event) {
        var ispitStakvaId = $(this).attr("ispitStavkaId");
        var ocjena = $("#" + ispitStakvaId).val();
        $.get("/stavke/SnimiOcjenuInput?ispitStavkaId=" + ispitStakvaId + "&ocjena=" + ocjena, function (data, status) {
            $("#divID").html(data);
    });
});
}
