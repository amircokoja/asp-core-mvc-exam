...
function DodajAjaxEvente() {
    $(".inputValue").focusout(function () {
        var takmicenjeUcesnikId = $(this).attr("takmicenjeUcesnikId");

        var bodovi = $("#" + takmicenjeUcesnikId).val();

        $.get("/stavke/SnimiFocusOut?takmicenjeUcesnikId=" + takmicenjeUcesnikId + "&rezultat=" + bodovi, function () {
            $("#divID").html(data);
        });
    });
